...
@Component({
  ...
  imports: [ReactiveFormsModule, NgSelectComponent, NgSelectModule, NgFor, NgIf],
...
export class FormUsersComponent implements OnInit {
  ...
  constructor(
    ...
    this.form = _formBuilder.group({
      ...
      password_confirmation: ['', [Validators.required, this._validatePasswordConfirmation]],
  ...
  public ngOnInit(): void {
    ...
    if (this.typeForm == TypeForm.UPDATE_PASSWORD) {
      this.typeForm = TypeForm.UPDATE_PASSWORD;
  ...
  public updateUserPassword(): void {
    this._usersService.updatePassword({
      id_user: this.id_user,
      full_name: '',
      username: '',
      password: this.form.get('password')!.value,
      ...
      next: (value: Response) => {
        if (value.code == 202) {
          this.decline();
          this._globalService.showToast(this.title, 'Contrasela de usuario actualizada exitosamente.', TypeToast.SUCCESS, 'checkmark-circle');
        } else if (value.code == 401) {
          this._globalService.showToast('Error', 'Sesión finalizada.', TypeToast.DANGER, 'code-slash');
        } else {
          this._globalService.showToast('Error', 'Ha ocurrido un error.', TypeToast.DANGER, 'code-slash');
